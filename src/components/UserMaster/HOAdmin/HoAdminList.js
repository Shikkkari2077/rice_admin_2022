...
class HoAdminList extends React.Component {
  state = {open: false,
    hideOld: false,
    ...
    fetch(Constant.getAPI() + "/users/list", {
      ...
      .then(function (json) {
        if (json.status === true) {
          ...
          that.setState({ hoAdminList: json.data });
        } else {
          that.setState({ hoAdminList: [] });
          Swal.fire({
...
openModel = () => {
  ...
  handleChange = (e, id) => {
    ...
    let newArray = this.state.hoAdminList.filter((d) => {
      // console.log(d)
                ...
                <div className="col-lg-4">
                  <div className="page-header-breadcrumb">
                    <ul className="breadcrumb-title">
                      ...
                      <li className="breadcrumb-item active">HO Admin List</li>
                    </ul>
                  </div>
                </div>
                </div>
