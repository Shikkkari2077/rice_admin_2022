...
class SalesRegister extends Component {
    ...
    const columns = [
      ...
      },
      {
        ...
        options: {
          ...
          customFilterListOptions: {
            render: (v) => {
              if (v[0] && v[1] && this.state.amountFilter) {
                return [`Min: ${v[0]}`, `Max: ${v[1]}`];
              } else if (v[0] && v[1] && !this.state.amountFilter) {
                return `Min: ${v[0]}, Max: ${v[1]}`;
              } else if (v[0]) {
                return `Min: ${v[0]}`;
              } else if (v[1]) {
          ...
          filterOptions: {
            ...
            display: (filterList, onChange, index, column) => (
              <div>
                <FormLabel style={{ fontSize: "12px" }}>Amount                </FormLabel>
                <FormGroup row>
                <input
                    label="min"
                    value={filterList[index][0] || ""}
